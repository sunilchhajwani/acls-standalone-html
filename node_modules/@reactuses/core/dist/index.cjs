...
const useRafFn = (callback, initiallyActive = true)=>{
    ...
    const step = React.useCallback((time)=>{
...
function useThrottleFn(fn, wait, options) {
    ...
    const fnRef = useLatest(fn);
    const throttled = React.useMemo(()=>lodashEs.throttle((...args)=>{
            return fnRef.current(...args);
        }, wait, options), // eslint-disable-next-line react-hooks/exhaustive-deps
    [
        wait,
...
const useMeasure = (target, options = defaultOptions$1)=>{
    const [rect, setRect] = React.useState(defaultState$1);
...
const useOnceEffect = createOnceEffect(React.useEffect);

const useOnceLayoutEffect = createOnceEffect(React.useLayoutEffect);
...
const useWindowSize = ()=>{
    ...
    const cached = index_js.useSyncExternalStore(subscribe, ()=>{
        const data = {
